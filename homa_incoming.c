...
void homa_add_packet(struct homa_rpc *rpc, struct sk_buff *skb)
{
	...
	int ipid = (int) ntohs(((struct iphdr *)skb_network_header(skb))->id);
	int offset = ntohl(h->seg.offset);
	int data_bytes = ntohl(h->seg.segment_length);
	struct sk_buff *skb2;
	struct homals_context *homals_ctx = rpc->homals_ctx;
	int homals_conf = homals_get_rx_conf(homals_ctx);
	...
	int offset2 = rpc->msgin.total_length - rpc->msgin.bytes_remaining;
	int data_bytes2 = 0;

	homals_prinf_int("%s 1. floor %d ceiling %d", __FUNCTION__, floor, ceiling);

	if (homals_conf) {
		homals_prinf_int("%s ipid %u", __FUNCTION__, ipid);
		homals_prinf_int("%s 1. skb->len %d skb->data_len %d skb_is_nonlinear %d", __FUNCTION__, skb->len, skb->data_len, skb_is_nonlinear(skb));
		data_bytes = skb->len - sizeof(struct data_header);
		if (unlikely(data_bytes < 0)) data_bytes += sizeof(struct data_segment);
		offset = rpc->msgin.total_length - rpc->msgin.bytes_remaining;
	}

	homals_prinf_int("%s offset %d data_bytes %d", __FUNCTION__, offset, data_bytes);
	homals_prinf_int("%s 1. rpc->msgin.bytes_remaining %d rpc->msgin.num_skbs %d", __FUNCTION__, rpc->msgin.bytes_remaining, rpc->msgin.num_skbs);
	...
	skb_queue_reverse_walk(&rpc->msgin.packets, skb2) {
		...
		data_bytes2 = skb2->len - sizeof32(struct data_header);
		if (unlikely(data_bytes2 < 0)) data_bytes2 += sizeof(struct data_segment);

		offset2 = (homals_conf) ? offset2 - data_bytes2 : ntohl(h2->seg.offset);

		homals_prinf_int("%s skb_queue_reverse_walk skb2 %px skb2->len %d", __FUNCTION__, skb2, skb2->len);
		homals_prinf_int("%s skb_queue_reverse_walk offset2 %d data_bytes2 %d", __FUNCTION__, offset2, data_bytes2);

		if (homals_conf) {
			int ipid2 = (int) ntohs(((struct iphdr *)skb_network_header(skb2))->id);
			if (ipid2 < ipid) {
	...
	homals_prinf_int("%s 2. floor %d ceiling %d", __FUNCTION__, floor, ceiling);
	...
	homals_prinf_int("%s __skb_insert skb %px skb2 %px skb2->next %px &rpc->msgin.packets %px", __FUNCTION__, skb, skb2, skb2->next, &rpc->msgin.packets);
	__skb_insert(skb, skb2, skb2->next, &rpc->msgin.packets);
	rpc->msgin.bytes_remaining -= (ceiling - floor);
	rpc->msgin.num_skbs++;

	homals_prinf_int("%s 3. floor %d ceiling %d", __FUNCTION__, floor, ceiling);
	homals_prinf_int("%s 2. rpc->msgin.bytes_remaining %d rpc->msgin.num_skbs %d", __FUNCTION__, rpc->msgin.bytes_remaining, rpc->msgin.num_skbs);
}
		...
		struct iov_iter *iter, int max_bytes)
{
	...
	skb_queue_walk_from(&msgin->packets, msgin->xfer_skb) {
		...
		homals_prinf_int("%s 1. skb %px this_offset %d this_size %d",
			__FUNCTION__, msgin->xfer_skb, this_offset, this_size);
			
		if (msgin->xfer_skb->len < sizeof(struct data_header)) {
			homals_prinf_int("%s skb %px skb->len %d",
			__FUNCTION__, msgin->xfer_skb, msgin->xfer_skb->len);
			break;
		}

		homals_prinf_int("%s 2. skb %px this_offset %d this_size %d",
			__FUNCTION__, msgin->xfer_skb, this_offset, this_size);
		...
		if (unlikely(this_size <= 0))
			continue;
		homals_prinf_int("%s skb %px offset %ld to %px len %d",
				__FUNCTION__, msgin->xfer_skb, 
		...
		struct homa_lcache *lcache)
{
	...
	homals_prinf_int("%s homals_ctx %px homals_conf %d", __FUNCTION__, homals_ctx, homals_conf);

	homals_prinf_int("%s in", __FUNCTION__);
	homals_prinf_int("%s 1. skb->len %d skb->data_len %d skb_is_nonlinear %d", __FUNCTION__, skb->len, skb->data_len,  skb_is_nonlinear(skb));

	homals_prinf_int("%s rpc->msgin bytes_remaining %d num_skbs %d", 
			__FUNCTION__, 
			rpc->msgin.bytes_remaining, 
			rpc->msgin.num_skbs);

	homals_prinf_int("%s incoming data packet, id %llu, peer 0x%x, offset %d/%d",
			__FUNCTION__,
	...
	homals_prinf_int("%s rpc->state != RPC_INCOMING %d", __FUNCTION__, rpc->state != RPC_INCOMING);	
	...
	if (rpc->active_links.next == LIST_POISON1) {
		...
		if (rpc->msgin.bytes_remaining == 0) {
			if (likely(homals_conf == HOMALS_SW))
				homals_sw_decrypt_conf(rpc, homals_ctx);
			homa_rpc_ready(rpc);
		}
		homa_sock_unlock(rpc->hsk);
	} else {
