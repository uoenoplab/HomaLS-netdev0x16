...
struct sk_buff *homa_fill_packets(struct homa_sock *hsk, struct homa_peer *peer,
		struct iov_iter *iter, struct homa_rpc* rpc)
{
	...
	size_t len = iter->count; // total message length in iovec iterator

	struct homals_context *homals_ctx = rpc->homals_ctx;
	int homals_conf = homals_get_tx_conf(homals_ctx);
	int homals_len = (homals_conf) ? HOMALS_RECORD_EXTRA_LENGTH : 0;

	homals_prinf_int("%s homals_ctx %px homals_conf %d homals_len %d", __FUNCTION__, homals_ctx, homals_conf, homals_len);
	...
	max_pkt_data = mtu - HOMA_IPV4_HEADER_LENGTH - sizeof(struct data_header); // TODO: fix max_pkt_data with homals
	if (len + homals_len <= max_pkt_data) {
		unsched = max_gso_data = len + homals_len;
		gso_size = mtu;
	} else {
		int bufs_per_gso;
		...
		if (unsched > (len + homals_len))
			unsched = (len + homals_len);
	}
	
	homals_prinf_int("homa_fill_packets stats: mtu %d max_pkt_data %d gso_size %d max_gso_data %d", mtu, max_pkt_data, gso_size, max_gso_data); // debug
	homals_prinf_int("homa_fill_packets stats: len %ld unsched %d homals_conf %d homals_len %d", len, unsched, homals_conf, homals_len); // debug
	 ...
	 * GSO in software.
	 */
	// loop for building packets
	for (bytes_left = len, last_link = &first; bytes_left > 0; ) {
		struct data_header *h;
		struct data_segment *seg;
		char *homals_record_extra_pre = NULL;
		char *homals_record_extra_post = NULL;
		...
		homals_prinf_int("%s 1. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));
		// setup GSO here
		// max_gso_data > max_pkt_data - no need to use GSO when max_gso_data is smaller than max_pkt_data
		// bytes_left > max_pkt_data - no need to use GSO when left bytes can fit into a single packet
		if (unlikely((bytes_left + homals_len > max_pkt_data)
		...
		homals_prinf_int("%s 2. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));

		skb_reserve(skb, HOMA_IPV4_HEADER_LENGTH + HOMA_SKB_EXTRA); // 60 bytes

		homals_prinf_int("%s 3. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));

		skb_reset_transport_header(skb);

		homals_prinf_int("%s 4. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));

		h = (struct data_header *) skb_put(skb,
				sizeof(*h) - sizeof(struct data_segment)); // 60-20 bytes for homa header (tcp-like)
		h->common.type = DATA;
		h->message_length = htonl(len + homals_len);
		
		homals_prinf_int("%s h->message_length %d len %ld homals_len %d", __FUNCTION__, ntohl(h->message_length), len, homals_len);

		available = max_gso_data;
		
		homals_prinf_int("%s 5. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));
		...
		do {
			int seg_size;
			int insert_homals_post = 0;
			int insert_homals_pre = 0;

			if (likely((homals_conf && (homals_record_extra_pre == NULL)))) {
				homals_record_extra_pre = skb_put(skb, HOMALS_RECORD_EXTRA_PRE_LENGTH);
				homals_record_extra_pre[0] = 0x17;
				homals_record_extra_pre[1] = 0x03;
				homals_record_extra_pre[2] = 0x03;
				available -= HOMALS_RECORD_EXTRA_PRE_LENGTH;
				insert_homals_pre = 1;
			}

			seg = (struct data_segment *) skb_put(skb, sizeof(*seg));

			homals_prinf_int("%s 0. available %d bytes_left %d seg_size %d", __FUNCTION__, available, bytes_left, seg_size);
			...
			if (likely(insert_homals_pre)) {
				if (seg_size + HOMALS_RECORD_EXTRA_PRE_LENGTH >= max_pkt_data)
					seg_size = max_pkt_data - HOMALS_RECORD_EXTRA_PRE_LENGTH;
			}

			homals_prinf_int("%s 1. seg->offset %d available %d bytes_left %d seg_size %d", __FUNCTION__, ntohl(seg->offset), available, bytes_left, seg_size);

			if (likely(homals_conf && (homals_record_extra_post == NULL) && (bytes_left - seg_size <= 0))) {
				available -= HOMALS_RECORD_EXTRA_POST_LENGTH;
				insert_homals_post = 1;
			}

			homals_prinf_int("%s 2. available %d bytes_left %d seg_size %d", __FUNCTION__, available, bytes_left, seg_size);
			...
			homals_prinf_int("%s 3. available %d bytes_left %d seg_size %d", __FUNCTION__, available, bytes_left, seg_size);

			if (likely(insert_homals_post)) {
				homals_record_extra_post = skb_put(skb, HOMALS_RECORD_EXTRA_POST_LENGTH);
				homals_record_extra_pre[3] = (homals_record_extra_post + HOMALS_RECORD_EXTRA_POST_LENGTH - homals_record_extra_pre) >> 8;
				homals_record_extra_pre[4] = (homals_record_extra_post + HOMALS_RECORD_EXTRA_POST_LENGTH - homals_record_extra_pre);
			}

			homals_prinf_int("homa_fill_packets data_seg loop: seg_size %d available %d bytes_left %d", seg_size, available, bytes_left);

		} while ((available > 0) && (bytes_left > 0));
		
		if (likely(homals_conf == HOMALS_SW))
			homals_sw_encrypt(skb, homals_ctx);

		homals_prinf_int("%s 7. head %px data %px trans %px tail %px len %u", __FUNCTION__, skb->head, skb->data, skb_transport_header(skb), skb_tail_pointer(skb), skb_headlen(skb));
		hexdump("homa_fill_packets skbuff ", skb->data, skb->len);
		...
		*last_link = NULL;
		homals_prinf_int("homa_fill_packets packet loop: available %d bytes_left %d", available, bytes_left);
	}
...
void homa_message_out_init(struct homa_rpc *rpc, int sport, struct sk_buff *skb,
		int len)
{
	int homals_conf = homals_get_tx_conf(rpc->homals_ctx);
	if (likely(homals_conf)) len += HOMALS_RECORD_EXTRA_LENGTH;
...
void homa_xmit_data(struct homa_rpc *rpc, bool force)
{
	while (rpc->msgout.next_packet) {
		...
		if (likely(homals_conf)) {
			homals_prinf_int("%s common.type %d", __FUNCTION__, 
					((struct data_header *)skb->data)->common.type);
			offset = homals_data_offset(skb);
		}
		...
		homals_prinf_int("homa_xmit_data offset %d rpc->msgout.granted %d", offset, rpc->msgout.granted);
		...
		homals_prinf_int("%s success return", __FUNCTION__);
	}
