...
static int __init homa_load(void) {
	...
	printk(KERN_NOTICE "Homa module (with homals addon) loading\n");
...
int homa_ioc_recv(struct sock *sk, unsigned long arg) {
	...
	rpc = homa_wait_for_message(hsk, args.flags, args.requestedId,
			&args.source_addr);
	homals_prinf_int("%s homa_wait_for_message done", __FUNCTION__);
	if (IS_ERR(rpc)) {
		err = PTR_ERR(rpc);
		rpc = NULL;
		goto error;
	...
	// if (homals_get_rx_conf(homals_get_ctx_hash(rpc->hsk, 
	// 	rpc->peer->addr, htons(rpc->dport)))) {
	// 	args.len -= HOMALS_RECORD_EXTRA_LENGTH;
	// }
	
	if (homals_get_rx_conf(rpc->homals_ctx))
		args.len -= HOMALS_RECORD_EXTRA_LENGTH;
	...
	if (rpc->homals_ctx) {
		result = homals_message_in_copy_data(rpc, &iter, iter.count);
	} else {
...
//		return err;
	...
	skbs = homa_fill_packets(hsk, peer, &iter, srpc);
	if (IS_ERR(skbs)) {
		err = PTR_ERR(skbs);
		goto done;
	}
	...
	if (!srpc) {
		homa_free_skbs(skbs);
		homals_prinf_int("%s (!srpc) err = -EINVAL", __FUNCTION__);
		err = -EINVAL;
		goto done;
	}
	if (srpc->state != RPC_IN_SERVICE) {
		homals_prinf_int("%s if (srpc->state != RPC_IN_SERVICE) err = -EINVAL", __FUNCTION__);
		err = -EINVAL;
		goto unlock;
	}
...
//		return err;
	...
	tt_record("homa_ioc_send call homa_rpc_new_client"); // debug
...
int homa_setsockopt(struct sock *sk, int level, int optname,
	...
	if (level == SOL_TLS) {
		return homals_setsockopt(sk, optname, optval, optlen);
	}
...
//					scaled_ms/10, scaled_ms%10);
	...
	*short_link = packets;
	...
	for (skb = packets; skb != NULL; skb = next) {
		...
		homals_prinf_int("%s 2. skb->len %d skb->data_len %d skb_is_nonlinear %d", __FUNCTION__, skb->len, skb->data_len,  skb_is_nonlinear(skb));
		...
		// TODO: add homals conf check
		if (unlikely((h->type == DATA)
				&& (skb->len < header_lengths[h->type-DATA]))) {
			homals_fake_data_segment = 1;
			skb->len += sizeof(struct data_segment);
		}
		...
		if (homals_fake_data_segment) skb->len -= sizeof(struct data_segment);
...
//				homa_log_throttled(homa);
		...
		// hexdump(__FUNCTION__, skb->data, skb_headlen(skb));
		homals_prinf_int("%s 3. skb->len %d skb->data_len %d skb_is_nonlinear %d", __FUNCTION__, skb->len, skb->data_len,  skb_is_nonlinear(skb));
